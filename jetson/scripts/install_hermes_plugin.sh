...
#
# Assumes Hermes was installed via its official installer to ~/.hermes/
# (code at ~/.hermes/hermes-agent, venv at ~/.hermes/hermes-agent/venv).
# Override paths with HERMES_HOME / HERMES_VENV if your layout differs.
...
if [ -d "$HERMES_VENV" ]; then
    "$HERMES_VENV/bin/pip" install -e "$REPO_ROOT/robot_bridge"
else
    echo "WARNING: Hermes venv not found at $HERMES_VENV — installing into system Python"
    pip3.11 install -e "$REPO_ROOT/robot_bridge" || pip3 install -e "$REPO_ROOT/robot_bridge"
fi
